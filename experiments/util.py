...
def initialize_config(config_name, continual=False):
    logger.info("Running experiment: {}".format(config_name))

    if continual:
        config = pyhocon.ConfigFactory.parse_file("experiments_continual.conf")[config_name]
    else:
        config = pyhocon.ConfigFactory.parse_file("experiments.conf")[config_name]

    config['log_dir'] = join(config["log_root"], config["dataset"]+"_"+config["language"]+"_"+config_name)
    makedirs(config['log_dir'], exist_ok=True)

    config['continual_save_dir'] = join(config["log_root"], "continual_"+config["dataset"]+"_"+config["language"]+"_"+config_name)
    makedirs(config['continual_save_dir'], exist_ok=True)
